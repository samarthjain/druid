...
DRUID_JAVA_OPTS="-Xmx256m -Duser.timezone=UTC -Dfile.encoding=UTF-8 -classpath conf/:$DRUID_USER_CLASSPATH:$HADOOP_CLASSPATH/* -Ddruid.hadoop.user.classpath=$DRUID_USER_CLASSPATH -Djava.library.path=$HADOOP_HOME/lib/native/ -Dhadoop.fs.s3n.impl=${FS_FQDN} -Dhadoop.fs.s3.impl=${FS_FQDN}"
